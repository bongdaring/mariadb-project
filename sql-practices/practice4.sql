...
select e.emp_no, CONCAT(e.first_name, ' ', e.last_name), max_salary.dept_name, max_salary.maxi
...
select e.emp_no as '사번', 
       ...
       CONCAT(dep_manage.first_name, ' ', dep_manage.last_name) as '매니저 이름', 
       dep_manage.dept_name as '부서'
	from employees e join dept_emp de on e.emp_no = de.emp_no
    join (	select d.dept_no, d.dept_name, e.emp_no, e.first_name, e.last_name
...
select e.emp_no, e.first_name, t.title, s.salary
    ...
    join salaries s on e.emp_no = s.emp_no
    join (	select dept_no, AVG(salary) AS avg_salary
				from dept_emp de join salaries s on de.emp_no = s.emp_no
			where 
				    de.emp_no = s.emp_no
                    AND de.to_date = '9999-01-01'
                    AND s.to_date = '9999-01-01'
			group by de.dept_no
			order by avg_salary desc limit 0, 1
