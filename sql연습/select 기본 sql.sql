...
select title from titles limit 10, 5; 

-- 예제4: titles 테이블에서 직금은 어떤 것들이 있는지 직급이름을 한 번씩만 출력하세요
...
-- 예제2: 논리연산자: employees 테이블에서 1989년 이전에 입사한 여직원의 이름, 성별, 입사일을 출력하세요.
select first_name, gender, hire_date
	 from employees
	where hire_date < '1991-01-01'
	 and gender='f'
...
-- 예제3: in 연산자: dept_emp 테이블에서 부서 번호가 d005이거나 d009에 속한 사원이 사번, 부서번호를 출력
...
-- 예제1: employees 테이블에서 직원 이름, 성별, 입사일 입사일 빠른 순으로 출력
